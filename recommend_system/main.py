...
async def recommend_related_books(
    author: str = Body(..., description="Author of the book to find related books for"),
    ...
    try:
        ...
        random_books = books_collection.aggregate([{"$sample": {"size": top_k}}])
        related_books = []
...
async def recommend_related_books_by_genre(
    genre: str = Body(..., description="Genre of the book to find related books for"),
    top_k: int = Body(5, description="Number of related books to return")
) -> List[dict]:
    try:
