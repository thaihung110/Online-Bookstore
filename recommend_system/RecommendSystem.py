...
class RecommendSystem:
    ...
    def fit(self, n_factors=20, lr=0.01, reg=0.01, n_epochs=10):
        try:
            ...
            ratings_cursor = ratings_col.find(
                {}, {"user_id": 1, "book_id": 1, "rating": 1, "created_at": 1}
            )
            ...
            insert_user_inx = []
            for u in user_map:
                insert_user_inx.append(
                    {"user_id": u, "user_index": user_map[u]}
                )
            insert_item_inx = []
            for i in item_map:
                insert_item_inx.append(
                    {"book_id": i, "item_index": item_map[i]}
                )
    ...
    def update(self, user_id, book_id, rating, created_at=None):
        try:
            ...
            if user_index is None:
                user_count = int(
                    self.db["user_index"].count_documents({})
                )  # Đếm số lượng user hiện tại

                # Thêm user mới vào user_index
                self.model.add_user()
                self.db["user_index"].insert_one(
                    {"user_id": user_id, "user_index": user_count}
                )
                self.model.P = np.vstack(
                    [
                        self.model.P,
                        np.random.normal(0, 0.1, (1, self.model.n_factors)),
                    ]
                )
            ...
            if item_index is None:
                ...
                self.model.add_item()
                self.db["item_index"].insert_one(
                    {"book_id": book_id, "item_index": item_count}
                )
                self.model.Q = np.vstack(
                    [
                        self.model.Q,
                        np.random.normal(0, 0.1, (1, self.model.n_factors)),
                    ]
    ...
    def batch_update(self):
        ...
        try:
            ...
            ratings_array = np.array(
                [
                    [r["user_id"], r["book_id"], r["rating"], r["created_at"]]
                    for r in ratings
                ]
            )
    ...
    def recommend(self, user_id, top_k=10):
        ...
        try:
            ...
            rated_items = self.db["user_rating"].find(
                {"user_id": user_id}, {"book_id": 1}
            )
            # join voi item_index de lay item_index
            rated_items = list(
                self.db["item_index"].find(
                    {
                        "book_id": {
                            "$in": [item["book_id"] for item in rated_items]
                        }
                    },
                    {"book_id": 1, "item_index": 1},
                )
            )
            ...
            recommendations = self.model.recommend(
                user_index, top_k=top_k, rated_items=rated_items
            )
            print(f"Recommendations for user {user_id}: {recommendations}")
            # Chuyển đổi chỉ số item sang book_id
            item_index_map = {
                item["item_index"]: str(item["book_id"])
                for item in self.db["item_index"].find()
            }
            recommendations = [
                item_index_map.get(item, None)
                for item in recommendations
                if item in item_index_map
            ]
